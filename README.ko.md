...
소스 코드와 문서는 [Apache License 2.0](LICENSE)으로 배포합니다.

생성된 앱 바이너리, APK/AAB 릴리즈 패키지, 서명 자료, Play Store 등록 자료, 스크린샷, 피처 그래픽, 앱 아이콘, 상표, 브랜딩 자산은 별도로 명시하지 않는 한 Apache License 2.0 재사용 범위에 자동 포함되지 않습니다. 자세한 범위는 [NOTICE](NOTICE)를 확인하세요.

서드파티 의존성은 각 의존성의 라이선스를 따릅니다.
